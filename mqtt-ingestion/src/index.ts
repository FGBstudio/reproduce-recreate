...
let currentIntervalMs = parseInt(env.BATCH_INTERVAL_MS);
const MAX_BUFFER_SIZE = 20000;

async function flushBuffers(): Promise<boolean> {
  if (isFlushing) {
    logger.debug('Flush already in progress, skipping this cycle');
    return false;
  }

  // Prevent OOM by capping buffers
  if (telemetryBuffer.length > MAX_BUFFER_SIZE) {
    const droppedCount = telemetryBuffer.length - MAX_BUFFER_SIZE;
    telemetryBuffer.splice(0, droppedCount);
    logger.error({ droppedCount, currentSize: telemetryBuffer.length }, 'Telemetry buffer exceeded limit. Dropped oldest points to prevent Out Of Memory.');
  }
  if (rawBuffer.length > MAX_BUFFER_SIZE) {
    const droppedCount = rawBuffer.length - MAX_BUFFER_SIZE;
    rawBuffer.splice(0, droppedCount);
    logger.error({ droppedCount, currentSize: rawBuffer.length }, 'Raw message buffer exceeded limit. Dropped oldest points to prevent Out Of Memory.');
  }
  ...
  if (rawBatch.length === 0 && telemetryBatch.length === 0) {
    return true; // Nothing to flush, count as success
  }
...
function scheduleNextFlush(): void {
  setTimeout(async () => {
    const success = await flushBuffers();
    
    const baseInterval = parseInt(env.BATCH_INTERVAL_MS);
    if (success) {
      if (currentIntervalMs !== baseInterval) {
        currentIntervalMs = baseInterval;
        logger.info({ intervalMs: currentIntervalMs }, 'Database writes succeeded. Resetting flush interval.');
      }
    } else {
      // Exponential backoff up to 60 seconds
      currentIntervalMs = Math.min(currentIntervalMs * 2, 60000);
      logger.warn({ intervalMs: currentIntervalMs }, 'Database writes failed. Backing off flush interval.');
    }
