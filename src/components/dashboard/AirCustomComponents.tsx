...
const getAqColor = (aq: string | number) => {
  ...
  if (isNaN(val)) return "text-slate-600";
               ...
               <div className="flex flex-col gap-2">
                 {Object.entries(METRICS_CONFIG).map(([key, info]) => (
                   <div key={key} className="grid grid-cols-4 gap-2 items-center border-t border-gray-800 pt-2">
                     <div className="col-span-1 font-semibold text-slate-600">{key.replace('iaq.', '').replace('env.', '')}</div>
                     <div className="text-slate-600">{info.legend.green}</div>
                     <div className="text-slate-600">{info.legend.yellow}</div>
                     <div className="text-slate-600">{info.legend.red}</div>
                   </div>
                 ))}
               </div>
             </div>
        ...
        <table className="w-full text-left border-collapse min-w-[600px] table-fixed">
          <thead>
            <tr className="border-b-2 border-gray-100/60 bg-gray-50/50">
              ...
              <th className="py-4 px-5 text-xs font-bold text-slate-600 uppercase tracking-widest whitespace-nowrap">°C Temp</th>
        ...
        <div className="min-w-max">
          ...
          {heatmapGrid.rows.map((row: any) => (
              <div key={row} className="flex items-center h-6 mb-0.5">
                  <div className="w-12 flex-shrink-0 text-[10px] text-slate-600 text-right pr-2">
