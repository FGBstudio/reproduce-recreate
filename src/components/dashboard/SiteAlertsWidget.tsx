                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {alert.metric && !alert.metric.startsWith('system.') && (
                        <div className="space-y-0.5">
                          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter flex items-center gap-1">
                            ...
                            <LegendTooltip 
                              iconSize={10} 
                              content="The exact value of the metric at the time the problem was detected by the micro-batch engine." 
                            />
                          </p>
                          <p className="text-xs font-black text-slate-800">
                             ...
                             {alert.duration_minutes !== undefined && alert.duration_minutes > 0 ? (
                               <span className="block text-[9px] text-slate-500 font-medium mt-0.5 leading-tight">
                                 (Sustained for {alert.duration_minutes}m)
                               </span>
                             ) : alert.duration_minutes === 1440 ? (
                               <span className="block text-[9px] text-slate-500 font-medium mt-0.5 leading-tight text-indigo-500 font-bold">
                                 (Daily Budget Exceeded)
                               </span>
                             ) : alert.duration_minutes === 0 ? (
                               <span className="block text-[9px] text-slate-500 font-medium mt-0.5 leading-tight text-rose-500 font-bold">
                                 (Instant Spike)
                               </span>
                             ) : null}
                          </p>
                      ...
                      <div className="space-y-0.5">
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter flex items-center gap-1">
                          ...
                          <LegendTooltip 
                            iconSize={10} 
                            content={`Alerts auto-resolve using a ${(alert.hysteresis_pct || 0)}% hysteresis buffer to prevent notification spam.`} 
                          />
                        </p>
                        <p className="text-xs font-black text-slate-800">
